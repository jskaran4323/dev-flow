...
public class ProjectUserController {
  ...
  public ResponseEntity<Map<String, String>> approveJoinRequest(
      @PathVariable UUID projectId, @RequestBody ApproveRequestDto request, Authentication auth) {
    try {
      CustomUserDetails userDetails = (CustomUserDetails) auth.getPrincipal();
      UUID ownerId = userDetails.getUser().getId();
  ...
  public ResponseEntity<Map<String, Boolean>> checkJoinRequestStatus(
    ...
    try {
      ...
      boolean hasRequest =
          joinRequestRepository.existsByProjectIdAndUserIdAndStatus(projectId, userId, "PENDING");
