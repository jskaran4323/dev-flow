...
public class IssueMapper {
  ...
  public static IssueResponse toIssueResponse(Issue issue) {
    List<LabelDto> labelDtos =
        Optional.ofNullable(issue.getLabels()).orElse(Collections.emptySet()).stream()
            .map(label -> new LabelDto(label.getId(), label.getType()))
            .toList();

    List<CommentDto> commentDtos =
        Optional.ofNullable(issue.getComments()).orElse(Collections.emptyList()).stream()
    ...
    if (issue.getAssignee() != null) {
      ...
      assigneeResponse =
          new ProjectUserResponse(
              assignee.getId(), assignee.getFullname(), assignee.getUsername(), userTypeEnum);
    }
  ...
  private static ProjectUserResponse toProjectUserResponse(User owner) {
    ...
    return new ProjectUserResponse(
        owner.getId(), owner.getFullname(), owner.getUsername(), owner.getUserTypeEnum());
  }
}
